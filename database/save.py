...
def salvar_no_banco(resultado: Dict) -> bool:
    ...
    Args:
        resultado: Dicionário retornado por processar_arquivo_drive()
    
    """
    # valida se a extração foi bem-sucedida se não pula documento
    ...
    try:
        ...
        # insere envolvidos com logico de fallback
        for envolvido in resultado['envolvidos']:
            ...
            if id_prt:
                ...
                print(f"  ✓ Envolvido inserido: {envolvido['representante']}")
            else:
                raise Exception(f"Falha ao inserir envolvido: {envolvido['representante']}")
